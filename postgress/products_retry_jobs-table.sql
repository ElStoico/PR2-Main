...
-- 3. Crear los índices apuntando a la tabla products_retry_jobs
CREATE INDEX idx_retry_jobs_product_id ON public.products_retry_jobs USING btree (product_id);

CREATE INDEX idx_retry_jobs_next_run_at_status ON public.products_retry_jobs USING btree (next_run_at, status)
WHERE
    (status = 'SCHEDULED');

CREATE INDEX idx_retry_jobs_status ON public.products_retry_jobs USING btree (status);

-- Índice simplificado con las columnas actuales
